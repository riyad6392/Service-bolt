...
class WorkerSchedulerController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $scheduleData = DB::table('quote')
            ->select('quote.*', 'customer.image', 'personnel.phone', 'personnel.personnelname', 'services.color')
            ->join('customer', 'customer.id', '=', 'quote.customerid')
            ->join('personnel', 'personnel.id', '=', 'quote.personnelid')
            ->where('quote.personnelid', $worker->workerid)
            ->join('services', 'services.id', '=', 'quote.serviceid')
            ->whereIn('quote.ticket_status', [0, 2, 3, 4])
            ->where('quote.personnelid', $worker->workerid)
            ->where('quote.givendate', $todaydate)
