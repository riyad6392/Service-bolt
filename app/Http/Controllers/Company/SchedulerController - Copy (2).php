...
class SchedulerController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $worker = Personnel::where('userid', $auth_id)->limit('6')->get();
    ...
    public function leftbarschedulerdata(Request $request)
    {
      ...
      for($i=8; $i<=23; $i++) {
        ...
        if($i>=12) {
          ...
          $html .='<li><div class="ev-calender-hours">'.$plusone.''.$colon.' <span>'.$ampm.'</span></div><ul class="d-flex w-100 connectedSortable ui-sortable sortable2" id="">';
          $f= $i+1;
              $m =   ":00";
          $timev = $f.$m .' '.$ampm;
          foreach($worker as $key => $value1) {
            $html .='<li class="d-inline-block ui-sortable-handle border-1 radius-5 slidescroll" style="width: 20%; border-radius:5px; border:1px solid #ccc; background:#fff; margin-right:10px;" id="'.$value1->id.'">
          ...
          foreach($scheduleData as $key => $value) {
              ...
              if($value->giventime == $f.$m .' '.$ampm) {
                  if($value->personnelid == $value1->id) {
                    $html .='<div><input type="hidden" id="drop_'.$value->id.'"><input type="hidden" name="customerid" id="customerid" value="'.$value->customerid.'"><input type="hidden" name="quoteid" id="quoteid_'.$value->id.'" value="'.$value->id.'"><span>#'.$value->id.'</span><h6 class="">'.$value->customername.'</h6><a href="javascript:void(0);" class="info_link1" dataval="'.$value->id.'"><i class="fa fa-trash" style="position: absolute;right: 56px;top: 20px;"></i></a><p>'.$value->servicename.'</p></div>';
   ...
   public function createquote(Request $request)
   {
    ...
    public function sortdata(Request $request)
    {
        ...
        DB::table('quote')->where('id','=',$quoteid)
          ->update([ 
              "ticket_status"=>"$tstatus","giventime"=>"$time","givendate"=>"$date","personnelid"=>"$workerid"
          ]);
    }
  ...
  public function view(Request $request ,$id) 
  {
    ...
    $pname = DB::table('personnel')->where('id',$id)->first();
    ...
    return view('scheduler.view',compact('scheduleData','pname'));
  }
   ...
   $fulldate =  $request->fulldate;
      ...
      for($i=8; $i<=23; $i++) {
        ...
        if($i>=12) {
          ...
          $html .='<li class=""><div class="ev-calender-hours">'.$plusone.''.$colon.' <span>'.$ampm.'</span></div><ul class="d-flex w-100">';
            $f= $i+1;
            $m =   ":00";
            $timev =  $f.$m .' '.$ampm;
