			  ...
			  <tbody>
                ...
                @foreach($inventoryData as $inventory)
			          ...
			          $twentyfivepercent = $inventory->pquantity*25/100;
			       	@endphp
				  <tr target="{{$i}}" data-id="{{$inventory->id}}" class="user-hover showSingle">
...
<div class="modal fade" id="edit-product" tabindex="-1" aria-labelledby="add-personnelModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content customer-modal-box">
      ...
      <div class="modal-body">
       	<form method="post" action="{{ route('worker.inventoryupdate') }}" enctype="multipart/form-data">
        @csrf
...
<script type="text/javascript">
  ...
  jQuery(function() {
   $(document).on('click','.showSingle',function(e) {
        ...
        $.ajax({
            url:"{{url('personnel/manageproducts/leftbarinventorydata')}}",
            data: {
              targetid: targetid,
              serviceid: serviceid 
            },
...
$(document).on('click','#editProduct',function(e) {
  $('.selectpicker').selectpicker();
   ...
   $.ajax({
            url:'{{route('worker.editviewinventorymodal')}}',
            data: dataString,
            method: 'post',
            dataType: 'json',
            refresh: true,
...
function readURL(input) {
 ...
 $('html').on('click','.info_link1',function() {
        ...
        function (isConfirm) {
          if (isConfirm) {
           $.ajax({
                url:"{{route('worker.deleteProduct')}}",
                data: {
                  id: productid 
                },
                method: 'post',
 ...
 $(document).ready(function () {
    $("#btnSubmit").click(function (event) {
      ...
      $.ajax({
        url:'{{route('worker.savefieldproduct')}}',
        data: data,
        processData: false,
        contentType: false,
        cache: false,
