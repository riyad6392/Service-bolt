  ...
  <div class="modal-dialog modal-dialog-centered ">
    <div class="modal-content customer-modal-box">
      <div class="modal-body">
        ...
        <form method="post" action="{{ route('worker.personnelupdate') }}" enctype="multipart/form-data">
        @csrf
    ...
    <div class="modal-content customer-modal-box overflow-hidden">
     <form class="form-material m-t-40  form-valide" method="post" action="{{route('worker.personnelcreate')}}" enctype="multipart/form-data">
      ...
      @enderror
	   ...
	   <div class="col-md-12 mb-3">
	  	<input type="text" class="form-control" placeholder="Password" name="wpassword" id="wpassword" value="{{ old('wpassword') }}" required="">
	   </div>
...
<div class="modal fade" id="edit-timesheet" tabindex="-1" aria-labelledby="add-personnelModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content customer-modal-box">
      <div class="modal-body">
<form method="post" action="{{ route('worker.timeupdate') }}" enctype="multipart/form-data">
        @csrf
...
<script type="text/javascript">
  $(document).ready(function() {
   ...
   function getschedulerdata(fulldate,id) {
    $.ajax({
            url:"{{url('personnel/managepersonnel/leftbarpersonnelschedulerdata')}}",
            data: {
              fulldate: fulldate,
              id:id
            },
   ...
   $(document).on('click','.showSingle',function(e) {
        ...
        $.ajax({
            url:"{{url('personnel/managepersonnel/leftbarservicedata')}}",
            data: {
              targetid: targetid,
              serviceid: serviceid 
            },
...
$(document).on('click','#editPersonnel',function(e) {
   ...
   $.ajax({
            url:'{{route('worker.viewpersonnelmodal')}}',
            data: dataString,
            method: 'post',
            dataType: 'json',
            refresh: true,
  ...
  $(document).on('click','#search',function(e) {
   ...
   var until = $("#until").val();
   $.ajax({
            url:"{{url('personnel/managepersonnel/leftbarpersonneltimesheetdata')}}",
            data: {
              id: id,
              from :since,
              to :until,
  ...
  $(document).on('click','#edittimesheet',function(e) {
   var id = $(this).data('id');
   $.ajax({
            url:"{{url('personnel/managepersonnel/leftbaredittimesheetdata')}}",
            data: {
              id: id
      },
            method: 'post',
  ...
  var today = yyyy + '-' + mm + '-' + dd;
   ...
   $.ajax({
            url:"{{url('personnel/managepersonnel/leftbarpersonneltimeoffdata')}}",
            data: {
              id: id,
              fulldate :today
      },
