    ...
    <div class="modal-content customer-modal-box">
     ...
     @if(count($serviceData)>0)
      @else
     	...
     	@endif
	 <form class="form-material m-t-40  form-valide" method="post" action="{{route('worker.checklistcreate')}}" enctype="multipart/form-data">
        @csrf
			    ...
			    <div class="row" >
			        ...
			        <div class="staresd">
			          <div class="imgup">
			            ...
			            <input type="file" class="form-control" style="margin-bottom: 5px;" name="checklistimage[]">
					  </div>
			        </div>
			      </div>
			    </div>
...
<div class="modal fade" id="edit-checklist" tabindex="-1" aria-labelledby="add-personnelModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content customer-modal-box">
      <div class="modal-body">
        <form method="post" action="{{ route('worker.updatechecklist') }}" enctype="multipart/form-data">
          @csrf
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$.ajax({
  			url:"{{url('personnel/checklist/leftbarchecklistdata')}}",
            data: {
              targetid: 0,
              serviceid: defalutid
            },
  ...
  $('html').on('click','.info_link1',function() {
      ...
      swal({
        ...
        function (isConfirm) {
          if (isConfirm) {
           $.ajax({
            url:"{{url('personnel/checklist/deleteChecklist')}}",
            data: {
              id: addressid 
            },
            method: 'post',
    ...
    $('html').on('click','#editchecklist',function() {
      ...
      var cname = $(this).data('name');
        $.ajax({
            url:'{{route('worker.vieweditchecklistmodal')}}',
            data: {
              'cid':cid,
              'cname':cname,
            },
