...
<div class="modal fade" id="add-category" tabindex="-1" aria-labelledby="add-customerModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content customer-modal-box">
     <form class="form-material m-t-40  form-valide" method="post" action="{{route('company.categoriescreate')}}" enctype="multipart/form-data">
        @csrf
...
<div class="modal fade" id="edit-category" tabindex="-1" aria-labelledby="add-personnelModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content customer-modal-box">
      <div class="modal-body">
      <form method="post" action="{{ route('company.categoriesupdate') }}" enctype="multipart/form-data">
        @csrf
...
$(document).on('click','#editCategory',function(e) {
  $('.selectpicker').selectpicker();
   ...
   $.ajax({
            url:'{{route('company.viewcategorymodal')}}',
            data: dataString,
            method: 'post',
            dataType: 'json',
            refresh: true,
  ...
  $('html').on('click','.info_link1',function() {
        ...
        function (isConfirm) {
          if (isConfirm) {
           $.ajax({
                url:"{{url('company/categories/delete')}}",
                data: {
                  id: customerid 
                },
                method: 'post',
