...
<div class="modal fade" id="add-services" tabindex="-1" aria-labelledby="add-personnelModalLabel" aria-hidden="true">
  ...
  <input type="text" class="form-control" placeholder="Service Default Price" name="price" id="price" onkeypress="return (event.charCode >= 48 && event.charCode <= 57) || event.charCode == 46 || event.charCode == 0" onpaste="return false" style="padding: 0 35px;" required="">
          ...
          <div class="col-lg-6 mb-2">
            <button type="button" class="btn btn-cancel btn-block" data-bs-dismiss="modal" aria-label="Close">Cancel</button>
          </div>
          <div class="col-lg-6">
...
<div class="modal fade" id="edit-services" tabindex="-1" aria-labelledby="add-personnelModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content customer-modal-box overflow-hidden">
      <form class="form-material m-t-40 form-valide" id="serviceformedit" method="post" enctype="multipart/form-data">
        @csrf
      <div class="modal-body">
        <div class="add-customer-modal d-flex justify-content-between align-items-center">
          <h5>Add a new Service</h5>
...
<!-- Edit product Modal -->
<div class="modal fade" id="edit-products" tabindex="-1" aria-labelledby="add-personnelModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content customer-modal-box overflow-hidden">
     <form class="form-material m-t-40 form-valide" id="productformedit" method="post" enctype="multipart/form-data">
        @csrf
      <div class="modal-body">
        <div class="add-customer-modal d-flex justify-content-between align-items-center">
        <h5>Add a new Product/Part</h5>
      ...
      var url = "{{url('company/services/createservice')}}";
       $.ajax({
            ...
            success:function(data) {
              ...
              $("#serviceidnew").append("<option value="+data.id+">"+data.servicename+"</option>");
              $("#serviceidnew").selectpicker('refresh');
              $("#edit-customer").show();
            }
        })
  });
    ...
    $('#productformedit').on('submit', function(event) {
        ...
        var url = "{{url('company/inventory/create')}}";
         $.ajax({
              ...
              success:function(data) {
                swal("Done!", "Product Created Successfully!", "success");
                $("#edit-products").modal('hide');
                $("#productidnew").append("<option value="+data.id+" data-price="+data.price+">"+data.productname+"</option>");
                $("#productidnew").selectpicker('refresh');
                $("#edit-customer").show();
              }
          })
    });
