  ...
  @foreach($worker as $key => $value)
    ...
    @endphp
      <div class="ticketbox border-end border-top" id="{{$value->id}}" class="text-center">
        <a style="color: #000!important;  text-decoration: none!important;font-size: 15px!important; display:block; margin-bottom:5px" href="{{url('company/scheduler/detail/')}}/{{$value->id}}"> <img class="sub" src="{{$imageurl}}" style="margin:0px;border-radius: 39px;height: 70px;width: 70px;"></a>
           <h5 style="text-align: center;font-size: 18px;color: black;"><a style="color: #000!important;  text-decoration: none!important;font-size: 15px!important;" href="{{url('company/scheduler/detail/')}}/{{$value->id}}">{{$value->personnelname}}</a></h5> 
      </div>
    @endforeach
  </div>
  <!-- endrightsidedata -->
...
<!-- <div class="custom-calender">
   <div class="ev-calender-title">
   <h3><span id="spanid" style="pointer-events: none;"><input type="hidden" name="dateval" id="dateval" value="{{ date('l - F d, Y') }}">{{ date('l - F d, Y') }}</span></h3>
   <span id="spanid1" saajtyle="pointer-events: none;position: absolute;top: 2px;background: #fb6794;
    color: #fff;border-radius: 17px;padding: 0 8px;"><input type="hidden" name="countval" id="countval" valaajue="{{ count($scheduleData) }}">{{ count($scheduleData) }}</span>
...
<script type="text/javascript">
 ...
 $(document).on("click","#suggest_trip_prev",function() {
    ...
    //alert(start);
      $.ajax({
       url:"{{url('company/scheduler/leftbarschedulerdataprev')}}",
       method:"POST",
       data:{"start":start,"fulldate":fulldate},
       dataType:"json",
     success:function(data) {
